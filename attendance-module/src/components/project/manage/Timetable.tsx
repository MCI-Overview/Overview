...
function getWeekRange(weekOffset: number): moment.Moment[] {
    ...
    return Array.from({ length: 14 }, (_, i) => startOfWeek.clone().add(i, 'days'));
}
        ...
        (name: string, date: string, item: { name: string } | null) => {
            setSlots(prevSlots => {
                ...
                if (item === null) {
                    ...
                    const existingSlotIndex = personSlots.findIndex(slot => slot.date === date);

                    if (existingSlotIndex !== -1) {
                        // Update existing slot
                        personSlots[existingSlotIndex] = { ...item, personName: name, date };
                    } else {
                        // Add new slot
                ...
                {NAMES.map((name, nameIndex) => (
                    <Grid container key={nameIndex} spacing={0}>
                        ...
                        {currentWeekRange.map((date, dateIndex) => {
                            const slot = slots[name]?.find(slot => slot.date === date.format('MM/DD'));
                            return (
                                <Grid xs key={dateIndex}>
                                    <Dustbin
                                        accept={[ItemTypes.GLASS, ItemTypes.PAPER, ItemTypes.FOOD]}
                                        lastDroppedItem={slot || null}
                                        onDrop={(item) => handleDrop(name, date.format('MM/DD'), item)}
                                        name={name}
