...
const AttendanceExportModal: React.FC<ModalProps> = ({ show, onClose, projectCuid }) => {
    ...
    const getExportData = async (formattedStartDate: string, formattedEndDate: string) => {
    ...
    const handleExport = async () => {
        ...
        const formattedData = data.map((entry) => ({
            ...
            name: entry.name,
            status: entry.status,
            shiftStart: entry.shiftStart ? dayjs(entry.shiftStart).format('HH:mm') : '',
        ...
        XLSX.writeFile(workbook, `attendance_${formattedStartDate}_to_${formattedEndDate}.xlsx`);
